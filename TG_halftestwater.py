...
while True:
    try:
        ...
        # 获取当前时间
        nowtimeday = (datetime.now() + datetime.timedelta(days=-1)).strftime('%Y-%m-%d')
        # 根据结果检查当前列表并导出数据
        ...
        # 如果当前比赛为空，则异常等待稍后继续
        if len(marketlist) == 0:
            print('维护或故障等待5分钟后继续执行...-nowtime:{}'.format(datetime.now().strftime(
                '%y-%m-%d %H:%M:%S')))
            time.sleep(300)
            # 登录系统
            login_page = tg.get('https://m3.tg6666.net/login.php', verify=False)
        ...
        db.close()
        # 完成一次记录等待
        print('等待10分钟后继续执行...-nowtime:{}'.format(datetime.now().strftime(
            '%y-%m-%d %H:%M:%S')))
        time.sleep(600)
    except:
        print('故障等待5分钟后继续执行...-nowtime:{}'.format(datetime.now().strftime(
            '%y-%m-%d %H:%M:%S')))
        time.sleep(300)
        # 登录系统
        login_page = tg.get('https://m3.tg6666.net/login.php', verify=False)
