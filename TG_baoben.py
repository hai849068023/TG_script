...
while True:
    ...
    # 根据结果检查当前列表并导出数据
    overgame = tg.get('https://m3.tg6666.net/gameresult.php?day={}'.format('2020-10-14'), verify=False)
            ...
            if baoben == '保本':
                ...
                for gameodd in allgameodds:
                    ...
                    if is_baoben == 'display: block':
                        ...
                        has_balance = gameodd.select('.content_cell.table_trade.table_trade_align')[0].text.strip()
                        if has_odd != '':
                            print('可购买保本,赔率{},余额{}'.format(has_odd, has_balance))
                            # # 获取购买数据并下单操作
